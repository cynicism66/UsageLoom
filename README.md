...
- 周容量估算采用 v3 时间区间配对：保存额度查询时间、原始百分比、账号指纹、套餐及重置周期；迟到日志和用户确认补归属会重算对应区间，不重复累加。
...
升级前从托盘退出 UsageLoom，再安装新版 MSI 或解压 ZIP。两种版本默认共用 `%LOCALAPPDATA%\UsageLoom\user-data\`，请勿删除此目录。v2 估算保留为历史参考，不混入 v3 样本；旧版未保存的额度快照无法补造。美元结果是实验性 API 等价估算，不是官方美元余额。

详细说明见 [0.6.0 发布说明](docs/RELEASE-0.6.0.md)。安装包尚未签名；自动化测试不代表干净 Windows 安装、多屏/DPI 或真实托盘人工验收。下方截图仍为明确标注的 0.4.0 演示截图。
