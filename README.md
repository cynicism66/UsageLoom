...
0.7.8 相对于 0.7.7：启动就绪后立即重算采样起点，修复旧缓存进度滞后及连接断开被误判为强制重置的问题，支持严格核验后的空闲断线续接。[发布说明](docs/releases/RELEASE-0.7.8.md)

0.7.5 相对于 0.7.4：额度周期短暂变化后恢复原周期的有效样本，并增加脱敏查询来源诊断。[发布说明](docs/releases/RELEASE-0.7.5.md)

0.7.4 相对于 0.7.3：启动后立即恢复已保存的有效采样进度，不再等待首次批量计算才显示。[发布说明](docs/releases/RELEASE-0.7.4.md)

0.7.3 相对于 0.7.2：周容量采样进度随每个百分点更新，区分已确认与待确认，保留稳健估算门槛。[发布说明](docs/releases/RELEASE-0.7.3.md)

0.7.2 相对于 0.7.1：整理安装程序的快捷方式选择页，统一字体、对齐与按钮间距，增加应用图标。[发布说明](docs/releases/RELEASE-0.7.2.md)
