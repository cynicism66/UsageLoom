...
- **明确安全限制**：无法确认接续点的文件显示待核对数量，不猜测合并。存在待核对来源时暂缓新周容量估算、保留已有参考结果。首次安装或旧索引已丢失时，不能恢复分页日志中已裁掉的逐次历史。
...
详细说明见 [0.6.6 发布说明](docs/RELEASE-0.6.6.md)。安装包尚未签名；自动化测试不代表干净 Windows 安装、多屏/DPI 或真实托盘人工验收。下方截图仍为明确标注的 0.4.0 演示截图。
