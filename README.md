...
Claude 每周参考估算无法给出数字时，概览卡片直接显示具体原因；右侧信息图标现在可点击查看采样进度和适用边界。未被本机 Claude Code 日志覆盖的额度增长仍不会被换算成 Token。[发布说明](docs/releases/RELEASE-0.8.36.md)

概览里的 Codex／Claude 额度卡片统一标题、进度条与估算摘要的呈现方式，并移除“查看额度详情”按钮。Claude 的实验性估算诊断保留在额度详情页，概览只显示简洁摘要。[发布说明](docs/releases/RELEASE-0.8.35.md)

Claude 与 Codex 概览日期筛选按钮使用相同的间距，卡片排列保持一致。[发布说明](docs/releases/RELEASE-0.8.34.md)

Claude 额度卡片新增实验性的每周容量参考估算：将同一周期的连续额度快照与本机 Claude Code Token 日志配对；样本不足或来源不明确时直接说明原因，不把本机用量冒充全账号额度。[使用边界](docs/CLAUDE.md) · [发布说明](docs/releases/RELEASE-0.8.33.md)

Claude 与 Codex 的“自定义”日期筛选现在使用相同的“开始／结束”标签、`yyyy-MM-dd` 显示和窄窗口布局。[发布说明](docs/releases/RELEASE-0.8.32.md)
...
修复右上角“保存设置”遗漏 Claude 开关、目录和来源的问题，两个保存入口使用同一套配置保存逻辑。保存后切换页面或重启不再回到关闭状态。[发布说明](docs/releases/RELEASE-0.8.15.md)
...
0.7.5：额度周期短暂变化后恢复原周期的有效样本，并增加脱敏查询来源诊断。[发布说明](docs/releases/RELEASE-0.7.5.md)

0.7.4：启动后立即恢复已保存的有效采样进度，不再等待首次批量计算才显示。[发布说明](docs/releases/RELEASE-0.7.4.md)

0.7.3：周容量采样进度随每个百分点更新，区分已确认与待确认，保留稳健估算门槛。[发布说明](docs/releases/RELEASE-0.7.3.md)

0.7.2：整理安装程序的快捷方式选择页，统一字体、对齐与按钮间距，增加应用图标。[发布说明](docs/releases/RELEASE-0.7.2.md)

0.7.1：整理工程结构，并通过组件化依赖将未压缩发布体积缩小约 23.2%，保留自包含运行能力。[发布说明](docs/releases/RELEASE-0.7.1.md)
...
- **一键更新**：设置 → 软件更新，支持每日自动检查、发布说明、下载校验及升级重启，兼容 MSI/ZIP。[更新流程与恢复说明](docs/UPDATES.md)。首次获得此功能需要手动安装 0.6.7。
- **接续分页历史子集**：同一会话的完整计数序列按顺序匹配后，保留缓存中更完整的历史；后续追加继续校验，不按单个相同数值去重。
- **缩小估算暂停范围**：影响时间范围已知时，只跳过重叠估算区间；无法确定时仍保守暂停。

**重要的数据兼容性更新：建议使用旧版的用户升级，保留原用户数据目录，不要通过全量重建或清空数据库解决日志迁移问题。**

- **兼容分页日志迁移**：识别 `history_mode: paginated`，核对会话身份、序号和完整计数字段，在保留旧明细的基础上接续新增用量；重定时的累计快照不重复计费。
- **历史文件变化不再阻断全部扫描**：暂不能安全接续的来源保留旧索引和用量，其他正常会话继续采集；同会话的相关来源也受到保护。
- **明确安全限制**：无法确认接续点的文件显示待核对数量，不猜测合并。仅排除影响范围内的周容量估算区间，范围不明时暂停；保留已有参考结果。首次安装或旧索引已丢失时，不能恢复分页日志中已裁掉的逐次历史。
